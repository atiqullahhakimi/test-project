...
// پس از signUp (بدون سشن) یک بار Resend خودکار هم بزن
...
function buildRedirectURL() {
    return new URL("./", window.location.href).href; // /test-project/ یا :5500/
}
...
document.getElementById("signupBtn").onclick = async () => {
    ...
    console.log("signUp result:", { userId: data?.user?.id, email, redirectTo, error });

    if (error) {
        const msg = error.message.toLowerCase();
        if (msg.includes("already registered") || msg.includes("user already")) {
            const { error: reErr } = await supabase.auth.resend({ type: "signup", email });
            console.log("resend(signup) for existing user:", { email, reErr });
...
document.getElementById("signinBtn").onclick = async () => {
    ...
    if (current && current.email === email) { alert(`Already signed in as ${current.email}`); await refreshStatus(); return; }

    const { error } = await supabase.auth.signInWithPassword({ email, password });
    if (error) {
        const m = (error.message || "").toLowerCase();
        if (m.includes("invalid login credentials")) alert("Wrong email or password (or account not confirmed).");
        else if (m.includes("email not confirmed")) alert("Please confirm your email first. Check Inbox/Spam.");
...
if (resendBtn) {
    resendBtn.onclick = async () => {
        ...
        console.log("manual resend(signup):", { email, error });
...
document.getElementById("addClientForm").onsubmit = async (e) => {
    ...
    } catch (err) { if (err?.message !== "not-authenticated") console.error(err); }
};
